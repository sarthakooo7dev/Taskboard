...
const BoardCard = ({
  ...
  const isUpdateStatusOld =
    updateStatus.includes('w ago') || updateStatus.includes('mo ago')
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2  p-1">
        <div
          className={`w-12 h-8 rounded-lg flex items-center justify-center ${bg}`}
        >
          <Icon size={21} />
        </div>
        ...
        <div className="flex items-center gap-1 text-[11px] text-gray-500 ">
          ...
          <span
            className={`w-1.5 h-1.5 rounded-full ${
              isUpdateStatusOld ? 'bg-yellow-600' : 'bg-green-500'
            }`}
          />
        </div>
      </div>
