...
import { CheckCircle2, Loader2, MessageSquare, MoveRight, Pencil, StickyNote, UserPlus } from 'lucide-react';
...
const getActivityContent = (activity: Activity) => {
    ...
    switch (activity.type) {
        case "TASK_MOVED":
            return {
                icon: <MoveRight size={14} className="text-blue-400" />,
        ...
        case "COMMENT_CREATED":
            return {
                icon: <MessageSquare size={13} className="text-orange-400" />,
...
const ActivityTab = ({ boardId, taskId, isExpanded }: ActivityTabProps) => {
    ...
    const { data, isLoading, isFetching, } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (!res.ok) {
                ...
                throw new Error(
                    "Failed to fetch activity for the task",
                );
            }
            return res.json();
        },
    ...
    return (
        <div className="h-full overflow-y-auto minimal-scrollbar p-1 pl-2 ">
            ...
            {
                (isLoading) && <div className='h-full flex-1 flex items-center justify-center '>
            ...
            {ActivityData.map((val: Activity) => {
                ...
                return (
                    <div key={val.id} className="flex gap-2 items-center mt-[6px]"  >
                        <div className={`mt-0.5 flex h-6 w-6 items-center justify-center rounded-full ${bg_style}`}>
                            {icon}
                        </div>
                        ...
                        <div className="flex-1 flex justify-between items-center ">
                            ...
                            {isExpanded && <p className="mt-1 text-xs text-gray-500 pr-2">
